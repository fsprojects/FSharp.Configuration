...
let tests =
    testList "Ini File Provider tests" [
        testCase "Can return a string from the config file" (fun _ -> Expect.equal IniFileType.Section1.key2 "stringValue" "value")

        testCase "Can return an integer from the config file" (fun _ -> Expect.equal IniFileType.Section1.key1 2 "value")

        testCase "Can return a double from the config file" (fun _ -> Expect.equal IniFileType.Section2.key3 1.23 "value")
