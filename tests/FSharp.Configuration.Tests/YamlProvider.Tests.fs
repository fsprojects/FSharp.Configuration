...
let tests =
    testList "Yaml Config Provider tests" [
        ...
        """
            ...
            Expect.equal
                (List.ofSeq settings.Mail.ErrorNotificationRecipients)
                [ "user1@sample.com*"; "user2@sample.com*"; "user3@sample.com" ]
                "value"
...
let listTests =
    testList
        "Yaml Config Provider tests - lists"
        (Seq.toList
         <| testFixture (fun f () -> f(Lists())) [
             "Can load sequence of maps (single item)",
             fun settings ->
                 settings.LoadText
                     """
            items:
            ...
            """
             ...
             "Can load sequence of maps (multiple items)",
             fun settings ->
                 settings.LoadText
                     """
            items:
