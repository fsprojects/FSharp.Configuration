...
let tests =
    testList "App Settings Provider tests" [
        ...
        testCase "Can return a DateTime from the config file" (fun _ ->
            Expect.equal (Settings.TestDateTime.ToUniversalTime()) (DateTime(2014, 2, 1, 3, 4, 5, 777)) "value")
        ...
        testCase "Can return a guid from the config file" (fun _ ->
            Expect.equal Settings.TestGuid (Guid.Parse "{7B7EB384-FEBA-4409-B560-66FF63F1E8D0}") "value")
        testCase "Can read multiple connection strings from the config file" (fun _ ->
            Expect.notEqual Settings.ConnectionStrings.Test1 Settings.ConnectionStrings.Test2 "value")
    ]
