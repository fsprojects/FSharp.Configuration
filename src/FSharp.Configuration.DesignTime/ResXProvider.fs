...
let private toProperties (filePath: FilePath) resourceName : MemberInfo list =
    ...
    |> List.map(fun node ->
        ...
        let ty =
            node.GetValueTypeName Unchecked.defaultof<ITypeResolutionService>
            |> Type.GetType
...
let private createResXProvider typeName resourceName filePath =
    let ty =
        ProvidedTypeDefinition(thisAssembly, rootNamespace, typeName, baseType = Some typeof<obj>)

    ty.SetAttributes(ty.Attributes ||| TypeAttributes.Abstract ||| TypeAttributes.Sealed)
    toProperties filePath resourceName |> Seq.iter ty.AddMember
    ty

let inline private replace (oldChar: char) (newChar: char) (s: string) =
                ...
                fun typeName parameterValues ->
                    ...
                    | [| :? string as resourcePath |] ->
                        ...
                        if not(File.Exists filePath) then
                            invalidArg "file" "Resource file not found"
