...
type Helper() =
    static member CreateResizeArray<'a>(data: 'a seq) : ResizeArray<'a> =
        ResizeArray<'a>(data)
...
module Parser =
    type Scalar =
        ...
        static member FromObj(inferTypesFromStrings: bool) : obj -> Scalar =
            function
            | null -> String ""
    ...
    let parse(inferTypesFromStrings: bool) : (string -> Node) =
        ...
        fun text ->
            ...
            with
            | :? YamlDotNet.Core.YamlException as e when e.InnerException <> null -> raise e.InnerException // inner exceptions are much more informative
            | _ -> reraise()
    ...
    let update (target: 'a) (updater: Node) =
        let tryGetField x name =
            x.GetType().GetField(name, BindingFlags.Instance ||| BindingFlags.NonPublic)
            |> Option.ofNull

        let getChangedDelegate x =
            x
                .GetType()
                .GetField("_changed", BindingFlags.Instance ||| BindingFlags.NonPublic)
                .GetValue x
            :?> MulticastDelegate
        ...
        and updateList (target: obj) name (updaters: Node list) =
            maybe {
                ...
                let isSortable =
                    itemType.IsSubclassOf(typeof<Uri>)
                    || typeof<IComparable>.IsAssignableFrom (itemType)
        ...
        and updateMap (target: obj) name (updaters: (string * Node) list) =
            ...
            match
                updaters
                |> List.collect(fun (name, node) -> update target (Some name) node)
            with
            | [] -> []
...
type Root(inferTypesFromStrings: bool) =
    ...
    member x.Load(reader: TextReader) =
        try
            reader.ReadToEnd()
            |> Parser.parse inferTypesFromStrings
            |> Parser.update x
        with e ->
            async { errorEvent.Trigger e } |> Async.Start
            reraise()
    ...
    member x.LoadAndWatch(filePath: string) =
        ...
        Helper.File.watch true filePath
        <| fun _ ->
